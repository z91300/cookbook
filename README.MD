...
- **结构化详情**：难度 / 耗时 / 热量 / 适合时段一目了然；食材清单（名称 + 用量）、所需炊具、分步骤图文（每步可配多图）、小贴士
...
- 上传的栅格图自动重编码为 **WebP**（无体积收益时保留原格式），内容 `sha256` 去重（同内容秒传，不重复占库）
- 文件本体直接存数据库（PostgreSQL `BYTEA`），**备份 = 全库备份**，无需单独的附件磁盘卷
- 列表小图统一走 `?w=360` 缩略图（服务端内存缓存 + ETag 长缓存），打开快、流量省
- 上传类型按**文件魔数**识别（不采信客户端声明），只有栅格图/视频以 `inline` 直出，其余一律附件下载 + `nosniff`，杜绝同源存储型 XSS
